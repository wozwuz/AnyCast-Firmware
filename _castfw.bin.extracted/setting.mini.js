...
var Stream_array = ["game","video"];
var Stream_list_value=new Array (0,1);
var Stream_index = 0;
var current_Stream_index = -1;
var Stream_current_value = "game";
var Stream_current_value_num = 0;
...
var ezairmode_index = 0,ezairmode_select = 0,ezairmode_select_txt, ezarimode_txt="EZAir mode";
var ezairmode_array = ["Mirror+Stream","Mirror Only", "Auto"];
var compatible_index = 0,compatible_select = 0,compatible_select_txt, compatible_txt="Compatible";
var compatible_array = ["ON","OFF"];
var upgrade_bar = 0;
var ota_auto_flag=0;
...
var language_browser = navigator.language.toLowerCase();
var lanArr = ["en","fr","de","es","pl","zh-cn","zh-tw","ja","ko","it","cs","da","ru","nl","fi","nb","pt","hu","ro","sk","tr","sv","el","ar","id","he","th","fa"];
var display_txt="";
var magicLink_flag = false;
var magicLink_router_flag = false;

var iosmirrormode_index = 0, iosmirrormode_select = 0, iosmirrormode_select_txt="Mirror+Stream", iosmirrormode_txt="iOS Docking mode";
var iosmirrormode_array = ["Mirror+Stream","Mirror Only"];
...
function To_wifilist() {
    ...
    if(cur_url.indexOf("wifi") >= 0) console.log("already redirect from wifilist page");
    else {
        console.log("to wifilist page in 0.1 sec.");
        setTimeout(function(){
            history.pushState(null, "auto wifi redirect", "websetting.html?redirect=wifi");
...
function updata_button_mutilanguage(){
    ...
    $('#set_devname_ok').text(ok_string);
    $('#set_devname_cel').text(cancle_string);
    $('#ezairmode_ok').text(ok_string);
    $('#ezairmode_cel').text(cancle_string);
	
    $('#compatible_ok').text(ok_string);		
    $('#compatible_cel').text(cancle_string);
    ...
    $('#set_Stream_ok').text(ok_string);
    $('#set_Stream_cal').text(cancle_string);
    $('#set_autoplay_ok').text(ok_string);
    $('#set_autoplay_cel').text(cancle_string);
    $('#iosmirrormode_ok').text(ok_string);
    $('#iosmirrormode_cel').text(cancle_string);
}
//¶ÁÈ¡¶à¹úÓï ,ÆÚËüÆ½Ì¨
...
function Get_multi_language() {
    ...
    $.get(cgiurl, {type: 'get_webmultilanguage_text'},function(index_text_string) {
        ...
        if (index_text_string.length) {
            ...
            iosmirrormode_array[1] = ezairmode_array[1];
            $('label[for=mode_0]').text(ezairmode_array[1]); //Mirror only
            text_string=text_string_global;	
            ezairmode_array[0] = Get_text_from_textstring(text_string);
            iosmirrormode_array[0] = ezairmode_array[0];
            $('label[for=mode_1]').text(ezairmode_array[0]); //Mirror+Streaming
			
            $("#compatible_title").text(compatible_txt);
            $('label[for=compatible_0]').text(compatible_array[0]); 
            $('label[for=compatible_1]').text(compatible_array[1]); 
            ...
            $('#autoplaymode_warn_txt').text(changes_effect_string);
            $('#dvn_warn_txt1').text(changes_effect_string);
            text_string = text_string_global;
			...
			text_string=text_string_global;
			display_txt = Get_text_from_textstring(text_string); //ios display.
            $("#stream_res_txt").text(display_txt);
            text_string=text_string_global;	
            Stream_array[1] = Get_text_from_textstring(text_string);
            $('label[for=Stream_1]').text(Stream_array[1]); //video: HD
            text_string=text_string_global;	
            Stream_array[0] = Get_text_from_textstring(text_string);
            $('label[for=Stream_0]').text(Stream_array[0]); //game: fluent
            //text_string = text_string_global;
            Stream_current_value = Stream_array[parseInt(Stream_current_value_num)];
            $('#Stream_res_sel').text(Stream_current_value);

            text_string=text_string_global; //autoplay
            $("#autoplay_title").text(Get_text_from_textstring(text_string));

            text_string=text_string_global; //ezchannel
            $("#autoplay_ezchannel_txt").text(Get_text_from_textstring(text_string));

            text_string=text_string_global; //playlist
            $("#autoplay_playlist_txt").text(Get_text_from_textstring(text_string));

            text_string=text_string_global; //ssid
            $("#autoplay_ssid_txt").text(Get_text_from_textstring(text_string));

            text_string=text_string_global; //volume
            $("#autoplay_volume").text(Get_text_from_textstring(text_string));

            text_string=text_string_global; //wait time
            $("#autoplay_waittime").text(Get_text_from_textstring(text_string));

            text_string=text_string_global; //device name
            var lan_device_name = Get_text_from_textstring(text_string);
            $("#deviceName_txt").text(lan_device_name);
            $("#le_devicename").text(lan_device_name+" : ");
            $("#autoplay_ssid_txt").text(lan_device_name);
	    text_string=text_string_global;
	    $("#compatible_title").text(Get_text_from_textstring(text_string));		
			...
			$('#stream_res_txt').text("Display");
        }
		
    },"text");
}

function Get_EZCast_Mirascreen() {
    var cgiurl = "cgi-bin/wifi_info_GET.cgi?timeStamp=" + new Date().getTime();
    $.get(cgiurl, {type: 'CONFIG_EZCHANNEL_JS'},function(res) {
        if(Number(res) == 1) {
            console.log("EZCast Dongle");
...
function Set_autoplay_ui(){
    var cgiurl = "cgi-bin/wifi_info_GET.cgi?timeStamp=" + new Date().getTime();
    $.get(cgiurl, {type: 'getAutoplayEnable'},function(res) {
        console.log("Autoplay Enable : "+res);
        if(Number(res) >= 0) $("#Autoplay_val option[value="+res+"]").attr("selected","selected");
        if(Number(res) == 0) {
            $("#autoplaymode_txt").text("OFF");
            $("#autoplay_visible").css("display","none");
        }
        else if(Number(res) == 1) {
            $("#autoplaymode_txt").text("ON");
            $("#autoplay_visible").css("display","block");
        }
    },"text");

    $.get(cgiurl, {type: 'getAutoplaylist'},function(res) {
        console.log("Autoplay Playlist : "+res);
        if(Number(res) >= 0) $("#Autoplay_playlist option[value="+res+"]").attr("selected","selected");        
    },"text");    

    $.get(cgiurl, {type: 'getssid'},function(res) {
        console.log("Autoplay SSID : "+res);
        if(Number(res) >= 0) $("#Autoplay_ssid option[value="+res+"]").attr("selected","selected");        
    },"text");

    $.get(cgiurl, {type: 'getAutoplayVolume'},function(res) {
        console.log("Autoplay Volume : "+res);
        if(res) $("#volume option[value="+res+"]").attr("selected","selected");
    },"text");    

    $.get(cgiurl, {type: 'getAutoplayWaitime'},function(res) {
        console.log("Autoplay Waittime : "+res);
        if(res) $("#waittime option[value="+res+"]").attr("selected","selected");
    },"text");
}

function Set_autoplay_value(){
    var autoplay_onoff = $("#Autoplay_val").val();
    var autoplay_playlist = $("#Autoplay_playlist").val();
    var autoplay_ssid = $("#Autoplay_ssid").val();
    var autoplay_volume = $("#volume").val();
    var autoplay_waittime = $("#waittime").val();
    $.post('cgi-bin/set_autoplay.cgi','ezchannel_cmd=SET_AUTOPLAY_ENABLE&ezchannel_open='+autoplay_onoff);
    $.post('cgi-bin/set_autoplay.cgi','ezchannel_cmd=SET_AUTOPLAY_LIST&ezchannel_open='+autoplay_playlist);
    $.post('cgi-bin/set_autoplay.cgi','ezchannel_cmd=SET_AUTOPLAY_HOST_AP&ezchannel_open='+autoplay_ssid);
    $.post('cgi-bin/set_autoplay.cgi','ezchannel_cmd=SET_AUTOPLAY_VOLUME&ezchannel_open='+autoplay_volume);
    $.post('cgi-bin/set_autoplay.cgi','ezchannel_cmd=SET_AUTOPLAY_WAITIME&ezchannel_open='+autoplay_waittime);
    if(autoplay_onoff == 0) $("#autoplaymode_txt").text("OFF");
    else if(autoplay_onoff == 1) $("#autoplaymode_txt").text("ON");
}
    ...
    if (ota_upgrade_string.length > 0) {
        ...
        if (warn_ret < 0) {
            ...
            if ((check_OTA_first_flag == 1) || (Upgrade_listener_flag == 1)) {
                ...
                $('#new_version').text(server_version_string);
...
function get_wifi_mode() {
    $.get(cgiurl, {type: 'get_wifi_mode'},function(wifi_mode) {
        ...
        if (wifi_mode_status > 0) {
            if (!wifi_connect_flag && !magicLink_router_flag) {
...
function Get_connected_ssid_tick() {
    ...
    $.get(cgiurl, {type: 'get_connected_ssid'},function(connected_ssid) {
        ...
        if (ret >= 0) {
            ...
            if ((connected_ssid.length && lan_con != 1) || magicLink_flag == true) {
                if (connected_ssid_flag == 0) {
                    $('#connected_ssid').text(connected_ssid);
                    $('#connected_ssid').show();
                    wifi_connect_flag = 1;
                    connected_ssid_flag = 1;
                    show_connections_num_id = self.setInterval(Get_new_version_text, 3000);//show_connections_num_id = self.setInterval(Get_local_version, 3000);
                }
            } else {
                //$('#connected_ssid').hide();
...
function Get_connected_ssid() {
    ...
    loopnum = 0;
    if(!magicLink_flag) get_connected_ssid_id = self.setInterval(Get_connected_ssid_tick, 3000);
}
        ...
        if (ota_upgrade_string.length) {
            ...
            $.get(cgiurl, {type: 'get_wifi_mode'},function(wifi_mode) {
                ...
                if (wifi_mode_status > 0) {
                    ...
                    //$('#new_version').hide();
                    if(!magicLink_flag) $('#new_version').text(not_wifi_connect_string);
                    else $('#new_version').text(No_new_version_string);
                }
            },"text");
        }
    },"text");
...
function Check_new_version_boot() {
    $.get(cgiurl, {type: 'get_wifi_mode'},function(wifi_mode) {
        wifi_mode_status = connected_ssid_flag;//wifi_mode.indexOf("9");
        if (magicLink_flag) wifi_mode_status = 1;
        if (wifi_mode_status > 0) {
...
function resolution_timetick_function() {
    ...
    if (show_waiting_tick == 0) {
        if (show_waiting_tick_id > 0) { $("#resolution_cancle").click(); }
    }
...
function Set_resolution_listener() 
{
    ...
    $("#resolution_cancle").click(function() {
		clearInterval(show_waiting_tick_id);
		show_waiting_tick_id = -1;
		var res = Number(current_resolution_index);
        switch(res){
            case 2:
                $('#res_value').val("2");
                resolution_index = 2;
            ...
            default:
                $('#res_value').val("");
                resolution_index = -1;
                $('#resolution_sel').text(res+" unexpect result "); //FMT_1920x1080_60P
        }
		Set_hdmimode();
		$("#Resolution").hide();
    });
...
function Stream_set_default_value() {
    var i=0;
    $('#Stream_value').val("" + Stream_current_value_num);
    $('#current_Stream_value').val("" + 0);
    Stream_index = current_Stream_index;
    $('#Stream_res_sel').text(Stream_array[Stream_index]);
    Stream_current_value = $('#Stream_res_sel').text();
}
...
function Set_Stream_res() {
    $.post('cgi-bin/set_Stream_res_POST.cgi', $('#config-Stream-form').serialize());
}
function Stream_timetick_function() {
    $('#Stream_tick').html(show_waiting_tick);
    if (show_waiting_tick == 0) {
        if (show_waiting_tick_id > 0) {
            clearInterval(show_waiting_tick_id);
            show_waiting_tick_id = -1;
           Stream_set_default_value();
...
function set_Stream_timetick() {
    if (show_waiting_tick_id > 0) {
        clearInterval(show_waiting_tick_id);
        show_waiting_tick_id = -1;
    }
    show_waiting_tick = 15;
    show_waiting_tick_id = self.setInterval(Stream_timetick_function, 1000);
}


function Set_Stream_listener() {
    $("#set_Stream_ok").click(function() {
        
  		  console.log("Stream_index =  " + Stream_index);
   		  console.log("current_Stream_index =  " + current_Stream_index);
   		  if(Stream_index == current_Stream_index)
          $("#Stream_Res").hide();
				else{
	        var temp = $('#Stream_value').val();
	        Stream_current_value_num = $('#Stream_value').val();
	        temp = temp + "OK";
	        console.log("Stream_ent  OK  " + temp);
	
	        $('#Stream_value').val("" + temp);
	
	        for(i=0;i<Stream_list_value.length;i++){
	            if(Stream_list_value[i]==Stream_index)
	                break;
	        }
	        $('#Stream_res_sel').text(Stream_array[i]);
			
	        Stream_current_value = $('#Stream_res_sel').text();
	        current_Stream_index=Stream_index;
	        Set_Stream_res();
	        $("#Stream_Res").hide();						
				}
    });

    $("#set_Stream_cal").click(function() {
		Stream_index = current_Stream_index;
			console.log("set_Stream_cal  " + Stream_current_value_num);
		$('#Stream_Res').hide();

    });

    $('input:radio[name="Stream_Res_choose"]').click(function() {
        var that = $(this).val();
        var b = $(this).prev().text();
        Stream_index = Number(that);
        $('#Stream_text').text(Stream_index);
        switch (Stream_index) {
            case 0:
                $('#Stream_value').val("0"); //FMT_1650x750_1280x720_60P
                break;
            case 1:
                $('#Stream_value').val("1"); //FMT_2750x1125_1920x1080_24P
                break;
        }
          console.log("select   "+Stream_index);

    });

    $('#Stream_all').click(function() {
		console.log("Stream_all  click"+Stream_index);
        $('input:radio[name="Stream_Res_choose"]').each(function() {
			console.log("each radio: "+$(this).val());
			// $(this).removeAttr("checked");
			$(this).attr("checked", " ");
        });
...
//set resolution css
function set_Stream_radio() {
    var that = $("input[type=radio][name=Stream_Res_choose][value=" + Stream_index + "]");
...
//Creat Stream list
function Creat_Stream_list() {
    for (var i = 0; i < Stream_array.length; i++) {
        var that=$("label[for=Stream_" + i + "]");
        that.text(Stream_array[i]);
        that.prev().val(Stream_list_value[i]);
    }
}
...
//read Stream value
function Get_Stream_Res() {

    var cgiurl = "cgi-bin/wifi_info_GET.cgi?timeStamp=" + new Date().getTime();
    $.get(cgiurl, { type: 'get_Stream_Res'},function(stream_res) {
		console.log("get_Stream_Res : "+stream_res);
		var res = stream_res.indexOf("1");
		if(res >= 0 ){
			console.log("Get_Stream_Res is video");
			$("#Stream_res_sel").text(Stream_array[1]);
			$('#Stream_value').val("1");
			Stream_index = 1;
		}
		else{
			res = stream_res.indexOf("0");	
			if(res >= 0 ){
	    	console.log("Get_Stream_Res is game");
			$("#Stream_res_sel").text(Stream_array[0]);//FMT_1024x768_60P    
			$('#Stream_value').val("0");
			Stream_index = 0;
			}
		}    	
...
function default_mode_listener() {
    $('#defmode_ok').click(function(e) {
        ...
        if (save_defaultmode_index != defaultmode_index) {
            ...
            $('#default_mode').text("" + defaultmode_array[defaultmode_index]);
...
function Get_compatible_ui() {
    var cgiurl = "cgi-bin/wifi_info_GET.cgi?timeStamp=" + new Date().getTime();
    $.get(cgiurl, {type: 'get_compatible'},function(ret) {
        var value = Number(ret);
	    console.log("Get_ezairmode_ui: "+value);
        compatible_index = value;
        compatible_select = compatible_index;
        set_compatible_style();
    }, "text");
}

function compatible_listener() {
    $("#compatible_ok").click(function(e) {
        set_compatible_ui();
        $("#compatible_mode").hide();
    });

    $("#compatible_cel").click(function(e) {
        if (compatible_select != compatible_index) {
            compatible_select = compatible_index;
        }
        $("#compatible_mode").css("display","none");
    });
	
	 $('input:radio[name="compatible_item"]').click(function() {
        var that = $(this).val();
        compatible_select = that;
        compatible_select_txt = compatible_array[compatible_select];
//		alert("0 compatible_select_txt "+compatible_select_txt+" compatible_select : "+compatible_select);
    });

    $("#compatible").click(function() {
	$("#compatible_mode").show();
        $('input:radio[name="compatible_item"]').each(function() {
            $(this).attr("checked", "");
        });
        set_compatible_style();
...
function ezairmode_listener() {
    ...
    $('input:radio[name="ezairmodemode_item"]').click(function() {
        ...
        ezairmode_select_txt = ezairmode_array[ezairmode_select];
    });
...
//autoplay button
function autoplay_listener() {
    $("#set_autoplay_ok").click(function(e) {
        Set_autoplay_value();
        $("#Autoplay_set").css("display","none");
    });
...
function set_ezairmode_style() {
    ...
    that.attr("checked", "checked");
    ezairmode_select_txt = ezairmode_array[ezairmode_index];
    if(Number(ezairmode_index) == 1) $("#ezairmode_txt").text($('#ezairmode label[for=mode_0]').text());
    else if(Number(ezairmode_index) == 0) $("#ezairmode_txt").text($('#ezairmode label[for=mode_1]').text());
    else $("#ezairmode_txt").text($('#ezairmode label[for=mode_2]').text());
}
...
function set_ezairmode_ui() {
    ...
    $.get(cgiurl, {type: str},function(ret) { console.log("set_ezairmode_ui:"+ezairmode_select)}, "text");
    $("#ezairmode_txt").text(ezairmode_select_txt);
    ezairmode_index = ezairmode_select;
}

function set_compatible_ui() {
    var cgiurl = "cgi-bin/wifi_info_GET.cgi?timeStamp=" + new Date().getTime();
    var str = compatible_select + "\n" + "set_compatible\n";
    $.get(cgiurl, {type: str},function(ret) { console.log("set_compatible_ui:"+compatible_select)}, "text");
    $("#compatible_txt").text(compatible_select_txt);
    compatible_index = compatible_select;
}

function set_compatible_style() {
    var that = $("input[type=radio][name=compatible_item][value=" + compatible_index + "]");
    that.attr("checked", "checked");
    compatible_select_txt = compatible_array[compatible_index];
     $("#compatible_txt").text(compatible_select_txt);
}
...
function change_deviceName() {
    var tmp="set_device_name";
    tmp=$('#devname').val()+"\n"+tmp;
    $.get("cgi-bin/wifi_info_GET.cgi", {type:tmp});    
}
function set_deviceName() {
    var regx = /^[A-Za-z0-9\#\-\@\_\s]+$/; 
    devnameValue = $('#devname').val();

    if (!regx.test(devnameValue)) $('#dvn_warn_txt').text(PSW_ALLOW_string);
    else if (devnameValue.length > 17) $('#dvn_warn_txt').text("The device name can not be more than 17 characters");
    else if (devnameValue.length == 0) $('#dvn_warn_txt').text("The device name can not be less than 1 character");
    else if (devnameValue == devnameValue_save) $('#dvn_warn_txt').text("Please enter a new name");
    else {        
        $('#deviceName_hint').text(devnameValue);
        $('#dvn_warn_txt').text("Update Success.");        
        $('#DeviceName').hide();
        change_deviceName();
    }
}
function Set_DN_listener() {
    $("#set_devname_ok").click(function() {
        set_deviceName();
    });
    
    $("#devname").keyup(function(){
        //alert(event.keyCode);
        if(event.keyCode==13){
            set_deviceName();
        }
    });
    $("#deviceName_sel").click(function() {
        $('#devname').val($('#deviceName_hint').text());
        $('#DeviceName').show();
    });
    $("#set_devname_cel").click(function() {
        $('#DeviceName').hide();
    });
}
function get_devicename() {
    var cgiurl = "cgi-bin/wifi_info_GET.cgi?timeStamp=" + new Date().getTime();
    $.get(cgiurl, {type: 'get_device_name'},function(devname) {
        console.log("get_devicename"+devname);
        $('#deviceName_hint').text(devname);
        $('#devname').val(devname);
        $('#dvn_warn_txt').text(PSW_ALLOW_string);
        devnameValue_save = $('#deviceName_hint').text();
    },"text");
}
...
function set_appsk() {
    ...
    else $('#psk_warn_txt').text(LIST_WARN3_string);//"Please enter a new password to save again");    
}
//ÃÜÂëÉèÖÃ°´¼üÏìÓ¦
        ...
        if (current_language_index != language_index) {            
            if(language_index == multiLanguage_array.length - 1){//multiLanguage_array.length - 1 === 28
                for(var i = 0 ; lanArr[i]; i++){
                    if(lanArr[i] == language_browser){
                        console.log("language_index: "+i);
                        language_index =i;
                        break;
                    }
                    else if(lanArr[i] == language_browser.substring(0,2)) {
                        console.log("language_index: "+i);
                        language_index =i;
                        break;
                    }
                }
                if(language_browser == 'zh-hk') language_index = 6;
                ...
                setTimeout(function(){
                    set_ezairmode_style();
                    set_iosmirrormode_style();
                    Get_new_version_text();//Get_local_version();                    
                    Get_connected_ssid_tick();
                    Get_Stream_Res();
                },1000);
            }
...
function Get_AP_mac_ip() {
    $.get("cgi-bin/wifi_info_GET.cgi", {type: '0\nget_mac_ip'},function(mac_wlan0_ip) {
        if (mac_wlan0_ip.length) {
            start = 0;
            text_string_length = 0;
            text_string = mac_wlan0_ip;
            text_string_length = mac_wlan0_ip.length;
...
function reset_reboot_listener(){
	...
	$('#reset_ent').click(function(e)   //reset to default
	{
		...
		$("#dialog2").show();
        var cgiurl = "cgi-bin/wifi_info_GET.cgi?timeStamp=" + new Date().getTime();
        $.get(cgiurl, {type:"reset_default"}, function(){}, "text");
	});	
...
function click_listener() {
    ...
    Get_iosmirrormode_ui();
    iosmirrormode_listener();

    $( "#Autoplay_val" ).change(function() {
        if($("#Autoplay_val").val() == 1) $("#autoplay_visible").css("display","block");
        else $("#autoplay_visible").css("display","none");
    });
...
function GET_EZWIRE_TYPE(){
    $.get("cgi-bin/wifi_info_GET.cgi", {type:"CONFIG_EZWIRE_TYPE"},function(str){
        console.log("CONFIG_EZWIRE_TYPE: "+str);
        if(Number(str)==11 || Number(str)==12) {
            $("#wifilist").css("display","none");
            $("#pass_sel").css("display","none");
            magicLink_flag = true;
            $("#bom").append(" - MagicLink");
            GET_MAGICLINK_UI();
        }        
    });
}
function GET_MAGICLINK_UI(){
    $.get("cgi-bin/wifi_info_GET.cgi", {type:"get_magicLink_UI"},function(str){
        console.log("CONFIG_MAGICLINK_UI: "+str);
        if( Number(str) == 2 ) {
            $("#upgrade").css("display","none");
        }
        else if ( Number(str) == 1 ) magicLink_router_flag = true;
    });
}
function GET_WIFI_DONGLE(){
    $.get("cgi-bin/wifi_info_GET.cgi", {type: 'get_wifi_mode'}, function(wifi_mode) {
        console.log("GET_WIFI_DONGLE: "+wifi_mode);
        if(Number(wifi_mode) == 1 || Number(wifi_mode) == -1) {
            $("#wifilist").css("display","none");
            $("#pass_sel").css("display","none");
            magicLink_flag = true;
            $("#bom").append(" - NoWifi");
...
function DualCast_Checking(){
    var cgiurl = "cgi-bin/wifi_info_GET.cgi?timeStamp=" + new Date().getTime();
    $.get(cgiurl, {type:"CONFIG_DUALCAST_ENABLE"}, function(str){
        if( Number(str) == 1 ) $("#iosmirror").css("display", "block");
    });
}
function Get_iosmirrormode_ui() {
    var cgiurl = "cgi-bin/wifi_info_GET.cgi?timeStamp=" + new Date().getTime();
    $.get(cgiurl, {type: 'get_iosmirrormode'},function(ret) {
        var value = Number(ret);
        console.log("Get_iosmirrormode_ui: "+value);
        iosmirrormode_index = value;
        iosmirrormode_select = iosmirrormode_index;
        set_iosmirrormode_style();
    }, "text");
}

//iosmirror button
function iosmirrormode_listener() {
    $("#iosmirrormode_ok").click(function(e) {
        set_iosmirrormode_ui();
        $("#iosmirrormode").hide();
    });

    $("#iosmirrormode_cel").click(function(e) {
        if (iosmirrormode_select != iosmirrormode_index) {
            iosmirrormode_select = iosmirrormode_index;
        }
        $("#iosmirrormode").css("display","none");
    });

    $('input:radio[name="iosmirrormodemode_item"]').click(function() {
        var that = $(this).val();
        iosmirrormode_select = that;
        iosmirrormode_select_txt = iosmirrormode_array[iosmirrormode_select];
    });

    $("#iosmirror").click(function() {
        $("#iosmirrormode").show();
        $('input:radio[name="iosmirrormodemode_item"]').each(function() {
            $(this).attr("checked", "");
        });
        set_iosmirrormode_style();    
    });
...
//set iosmirror css
function set_iosmirrormode_style() {
    var that = $("input[type=radio][name=iosmirrormodemode_item][value=" + iosmirrormode_index + "]");
    that.attr("checked", "checked");
    iosmirrormode_select_txt = iosmirrormode_array[iosmirrormode_index];    
    if(iosmirrormode_index) $("#iosmirrormode_txt").text($('#iosmirrormode label[for=mode_0]').text());
    else $("#iosmirrormode_txt").text($('#iosmirrormode label[for=mode_1]').text());
}

//set iosmirror value
function set_iosmirrormode_ui() {
    var cgiurl = "cgi-bin/wifi_info_GET.cgi?timeStamp=" + new Date().getTime();
    var str = iosmirrormode_select + "\n" + "set_iosmirrormode\n";
    $.get(cgiurl, {type: str},function(ret) {}, "text");
    $("#iosmirrormode_txt").text(iosmirrormode_select_txt);
    iosmirrormode_index = iosmirrormode_select;    
}
function delay_get_data_function() {
    ...
    Get_multilanguage();
    Get_resolution();
    if(!magicLink_flag) Get_password();
    get_devicename();
    Get_new_version_text();//Get_local_version();
    Get_AP_mac_ip();
